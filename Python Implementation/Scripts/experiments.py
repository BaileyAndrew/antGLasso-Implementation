...
##
def get_cms_for_betas_with_errorbars(
    betas_to_try: "List of L1 penalties to try",
    attempts: "Amount of times we run the experiment to average over",
    kwargs_gen: "Dictionary of parameters for generating random data",
    cm_mode: "`mode` argument for `generate_confusion_matrices`" = "Nonzero",
    ...
    if algorithms is None:
        algorithms = ["scBiGLasso", "antGLasso_heuristic", "EiGLasso", "TeraLasso"]
    
    # First axis indexes individual attempts, i.e. for average values we should mean
    # over this axis.
    # Second axis indexes over Psi0, Psi1, etc, i.e. the axes of the tensor data
    # Third axis indexes over the algorithm used
    # Fourth axis indexes over the regularization parameters used
    # Fifth and sixth axes define the confusion matrix
    Psis_cms = np.zeros((attempts, len(kwargs_gen['ds']), *betas_to_try.shape, 2, 2))
    for idx_alg, alg in enumerate(algorithms):
        if verbose:
            print(f"Trying algorithm: {alg}")
        for idx_b, b in enumerate(betas_to_try[idx_alg]):
            ...
            for attempt in range(attempts):
                ...
                if alg == "scBiGLasso":
                    ...
                    Psis = antGLasso(
                        Ys=Ys,
                        betas=[b for _ in kwargs_gen['ds']],
                        B_approx_iters=1000
                    )
                elif alg == "antGLasso_heuristic":
                    #T, S = calculate_empirical_covariance_matrices(Ys)
                    ...
                    Psis = antGLasso_heuristic(
                        [T, S],
                        sparsities=[b for _ in kwargs_gen['ds']],
                        B_approx_iters=1000
                    )
                elif alg == "EiGLasso":
                    Psis = EiGLasso(
                ...
                Psis_cms[attempt, :, idx_alg, idx_b, ...] = np.array([generate_confusion_matrices(
                    ...
                    Psis_gen[idx],
                    mode=cm_mode
                ) for idx in range(len(kwargs_gen['ds']))])
                # End of attempts loop
            # End of betas loop
        # End of algorithms loop
    return Psis_cms
...
def make_cm_plots_with_errorbars(
    ...
    # Fourth axis indexes over the regularization parameters used
    # Fifth and sixth axes define the confusion matrix
    num_attempts = Psis_cms.shape[0]
    num_dimensions = Psis_cms.shape[1]
    num_algorithms = Psis_cms.shape[2]
    num_betas = Psis_cms.shape[3]
    ...
    with plt.style.context('Solarize_Light2'):
        ...
        fig, axes = plt.subplots(figsize=(16, 8), ncols=num_dimensions)
        axes_names = [f"Axis {x+1}" for x in range(num_dimensions)]
        for confmats, ax, name in zip(np.swapaxes(Psis_cms, 0, 1), axes, axes_names):
            for idx_alg, alg in enumerate(algorithms):
                precisions = np.array([
                ...
                # Now the axes are [betas, attempts]
                
                avg_precisions = precisions.mean(axis=1)
                avg_recalls = recalls.mean(axis=1)
                std_precisions = precisions.std(axis=1)
                std_recalls = recalls.std(axis=1)
                ...
                ax.plot(avg_recalls, avg_precisions, label=alg, c=colors[idx_alg])
                def polyx():
                ...
                def polyy():
                    ...
                    for p in zip(lower_precisions[::-1]):
                        yield p
                if alg not in omit_errorbars_from:
...
def create_precision_recall_curves_with_errorbars(
    ...
    Psis_cms = get_cms_for_betas_with_errorbars(
        betas_to_try,
        attempts=attempts,
        kwargs_gen=kwargs_gen,
        algorithms=algorithms,
